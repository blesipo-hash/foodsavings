...
#### Ecosystem Services #### - Here on is not done and likely the code does not work, was just starting


ES_values <- read_excel("Parameters2.xlsx", sheet = "ES")%>% # ecosystem services from Costanza et al separated by ecosystem type: agriculture, forest, wetland and shrubland
  dplyr::select(ES, ForestValue_2021, AgValue_2021, WetlandValue_2021,GrasslandValue_2021)%>%
  summarise(forestES = sum(ForestValue_2021, na.rm =TRUE),# summing the value of all ES by ecosystem type
            agES = sum(AgValue_2021, na.rm =TRUE),
            wetlandES = sum(WetlandValue_2021, na.rm=TRUE),
            grassES =  sum(GrasslandValue_2021,na.rm=TRUE))

# These lines are to bring these values to your global environment for use in pipes below 
ForestES <- ES_values$forestES
AgES <- ES_values$agES
WetES <- ES_values$wetlandES
GrassES <- ES_values$grassES

Ag_conversion <- read_excel("Parameters2.xlsx", sheet = "Ag_conversion")%>% # percent of land converted to agriculture to diff types of ecosystems - from Gibbs et al 2010
  dplyr::select(Region, LandCover, Conversion)%>%
  spread(LandCover, Conversion)%>%
  replace_na(list(Plantations = 0, Water =0))

rice_yields <- read.csv("rice-yields.csv")%>% # data on rice yields in metric tones per hectare from ourworldindata.com 
  dplyr::select(country_title, Year, Yield_mt_ha)%>%
  filter(Year >= 2000)%>%
  group_by(country_title)%>%
  summarise(mean_yield_rice_ha = mean(Yield_mt_ha, na.rm=TRUE),
            cost_ha_rice = mean_yield_rice_ha*cost_mt_rice) # cost of a hectare of rice

# same as above but for wheat
wheat_yields <- read.csv("wheat-yields.csv")%>%
  dplyr::select(country_title, Year, Yield_mt_ha)%>%
  filter(Year >= 2000)%>%
  group_by(country_title)%>%
  summarise(mean_yield_wheat_ha = mean(Yield_mt_ha, na.rm=TRUE),
            cost_ha_wheat = mean_yield_wheat_ha*cost_mt_wheat)

# metric tones of beef production by country from ourworldindata.com
beef_production <- read_excel("beef.xlsx", sheet = "meat-production-tonnes")%>% 
  dplyr::select(country_title, Year, MetricTones)%>%
  filter(Year >= 2000)%>%
  group_by(country_title)%>%
  summarise(mean_mt_beef = mean(MetricTones, na.rm=TRUE))

# total land area by country from the world bank
area <-read_excel("beef.xlsx", sheet = "Area")%>%
  dplyr::select(country_title, Area_ha)

# percentage of land used for cattle ranching obtained from the world bank
pasture <- read_excel("beef.xlsx", sheet = "Pastureland")%>%
  dplyr::select(country_title, Year, Pastureland)%>%
  filter(Year >= 2000)%>%
  group_by(country_title)%>%
  summarise(mean_pasture_use = mean(Pastureland, na.rm=TRUE))


beef_yields <- area %>%
  left_join(pasture, by = "country_title")%>%
  left_join(beef_production, by = "country_title")%>%
  mutate(area_pasture_ha = Area_ha * mean_pasture_use, # to obtain beef yields by country first estimate the total land used for pasture
         mean_yield_beef_ha = mean_mt_beef / area_pasture_ha, # divide beef production by pasture land
         cost_ha_beef = mean_yield_beef_ha*cost_mt_beef)%>%  # multiply by cost of beef to estimate the cost of a "hectare of beef"
  dplyr::select(country_title, mean_yield_beef_ha, cost_ha_beef)

# total hectares saved if MDA is implemented
ha_saved <-fs_country %>% # these estimates are only for Lo et al guidelines
  group_by(country_title, Region)%>%
  summarise(FS = sum(Food_Savings))%>%
  left_join(rice_yields, by = "country_title")%>%
  left_join(wheat_yields, by ="country_title")%>%
  left_join(beef_yields, by ="country_title")%>%
  replace_na(list(cost_ha_rice = 0, cost_ha_wheat = 0, cost_ha_beef =0))%>%
  mutate(ha_saved = FS/ (cost_ha_rice *0.35)+(cost_ha_wheat*0.35)+(cost_ha_beef*0.3), # ha saved based on proportion of commodity consumed
         ha_saved = ifelse(cost_ha_rice == 0, FS /(cost_ha_wheat*0.7)+(cost_ha_beef*0.3), # if a commodity is equal zero then put more weight on the other commodities. Note this assumes people are consuming locally. Big assumption!!!
                           ifelse(cost_ha_wheat == 0, FS /(cost_ha_rice*0.7)+(cost_ha_beef*0.3),
                                  ifelse(cost_ha_beef ==0, FS /(cost_ha_rice*0.45)+(cost_ha_wheat*0.45), ha_saved))))

# estimates of ES saved based on how many hecatres of forest, wetlands and shrublands are NOT converted to agricultural land due to MDA 
ES_saved <- ha_saved %>%
  left_join(Ag_conversion, by = "Region")%>%
  mutate(ES_nat = (ha_saved*Forest)*ForestES + (ha_saved*Shrub)*GrassES + (ha_saved*Water)*WetES +(ha_saved*Plantations)*AgES, # ES from natural ecosystems
         ES_ag = ha_saved*AgES, # ES from agriculture
         ES = ES_nat - ES_ag)%>% # Net ES
  filter(!country_title == "Kiribati") # Kiribati does not produce rice, wheat or beef (or at least there are no data on yields for those commodities)
  
# ES at regional scale
ES_ha_saved_region <- ES_saved %>%
  group_by(Region)%>%
  summarise(ES_nat = sum(ES_nat),
            ES_ag =sum(ES_ag),
            ES = ES_nat - ES_ag)%>%
  print()

# ES at global scale
ES_saved_global <- ES_ha_saved_region %>%
  ungroup()%>%
  summarise(ES_nat = sum(ES_nat),
            ES_ag = sum(ES_ag),
            ES = ES_nat - ES_ag)%>%
...
ES_ha_saved_region <- ES_ha_saved_region %>% rbind(ES_saved_global)

#### Net Benefits (DALYs + Food Savings + Ecosystem Services - MDA) ####
# This code is good again

ES_saved <- ES_saved %>%
  dplyr::select(country_title, FS, ES)

# net benefits by country using low, med and high cost of MDA
NB_country <- Country_MDA_Lo %>%
  dplyr::select(country_title, Average_Cost100:High_Cost100)%>%
  left_join(fs_country, by = "country_title")%>%
  left_join(DALYs_Combined, by = "country_title")%>%
  #left_join(ES_saved, by = "country_title")%>%
  mutate(NB_Low = (DALY_USD + Food_Savings) - Low_Cost100,
         NB_Avg = (DALY_USD  +  Food_Savings) - Average_Cost100,
         NB_High = (DALY_USD  + Food_Savings) - High_Cost100)

# Net benefits combined at global scale
NB_global <- NB_country %>%
  ungroup()%>%
  summarise(NB_Low = sum(NB_Low, na.rm =TRUE),
            NB_Avg = sum(NB_Avg, na.rm =TRUE),
            NB_High = sum(NB_High, na.rm =TRUE))%>%
  print()


#### Plots MDA vs (Food Savings + DALYs + ES) ####
